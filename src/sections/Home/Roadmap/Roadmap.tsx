          ...
          <div className="border border-light-gray rounded-[10px] p-2 w-full grid grid-cols-1 sm:grid-cols-2 gap-2">
            {content.newRoadMap.allTabs?.map((tab, index) => (
              <button
                ...
                key={index}
                className={`px-6 lg:px-7 py-2 rounded-lg border text-river-blue ${
                  tab === activeTab
                    ? "text-white button-gradient"
                    : "border-transparent"
                }`}
              >
                <StyledLgText fontColor="!!font-semibold">{tab}</StyledLgText>
              </button>
            ))}
          </div>
          <div className="flex flex-col justify-center gap-4">
            {content.newRoadMap.allTabsContent[activeTab]?.map(
              (card, index) => (
                <div
                  key={index}
                  className="flex flex-col gap-2 border border-light-gray rounded-2xl md:rounded-[22px] text-river-blue p-4 sm:p-6 md:p-8 max-w-md sm:w-[28rem] w-full"
                >
                  ...
                  <ul>
                    {card.content.map((item, index) => (
                      <li
                        key={index}
                        className="flex gap-3 text-river-blue !font-medium"
                      >
                        <span>•</span>
                        <StyledLgText
                          fontColor="!!font-medium"
                          children={item}
                        />
                      </li>
                    ))}
